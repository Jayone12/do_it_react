...
[링크](https://github.com/Jayone12/do_it_react/blob/%232/arrow_function/05-01.js)
...
## 디바운스와 스로틀
디바운스와 스로틀은 ES6 문법이 아니며 두 개념은 서버의 데이터를 요청하는 등의 작업에서 생기는 부하를 크게 줄여주므로 알아두는 것이 좋다.

<br>

### 디바운스(debounce)
디바운스는 어떤 내용을 입력하다가 특정 시간 동안 대기하고 있으면 마지막에 입력된 내용을 바탕으로 서버 요청을 하는 방법이다.  
예로 검색창에 내용 입력할 때 하단에 검색 입력이 멈추면 연관 검색어 목록이 나온다.  
[링크](https://github.com/Jayone12/do_it_react/blob/%232/src/debounce.js)

<br>

### 스로틀(throttle)
디바운스 개념과 비슷하지만 `'입력되는 동안에도 바로 이전에 요청한 작업을 주기적으로 실행한다는 점'`이 다르다.
예로 무한 스크롤 기능이 구현된 곳에 사용된다.  
지연 시간 동안 스크롤이 움직이면 여러 번의 서버 요청이 발생하는데 스로틀은 첫 번째 요청이 지연 실행되는 동안에 중복된 요청을 무시하고  
실행 후에 첫 번째로 호출되는 요청을 동일하게 지연 실행하여 구간 내에서는 중복 요청 과정을 생략한다.  

1. 첫 번째 요청 실행 (지연시간 발생)
2. 지연시간 동안 내 발생하는 요청은 무시
3. 위 내용을 반복

[링크](https://github.com/Jayone12/do_it_react/blob/%232/src/throttle.js)
